...
\end{equation}
Mit Gleichung \ref{AQ} lässt sich nun eine Beziehung  zwischen $[A]$ und der Schichtsteifigkeitsmatrix $[Q]$ finden.
\begin{equation}
\label{AQ}
	A_{ij}= \sum_{k=1}^{2} \overline{Q}_{ij,k}\cdot t_{k}
\end{equation}
Als nächstes muss die Steifigkeitsmatrix noch in die Richtung der Einzelschichten mit dem Faserwinkel $\alpha_{k}$ und der Transformationsmatrix $[T]$ transformiert werden.\\
\begin{equation}
...
\end{equation}\\
\noindent
Nun lässt sich mit Hilfe der E-Moduln, Querkontraktionszahlen und der Steifigkeitsmatrix $[Q]$ die Spannung in den einzelnen Fasern bestimmen.
\begin{gather}
\label{Faserspannung}
\begin{Bmatrix}
\sigma_{1}\\
...
\end{equation}
\noindent
Der E-Modul parallel (siehe Glg. \ref{Eparallel}) und orthogonal (siehe Glg. \ref{Eorthogonal}) kann mit den E-Modulen von Faser und Matrix, sowie dem Faservolumenanteil $\varphi$ errechnet werden.
\begin{equation}
\label{Eparallel}
