...
\begin{equation}
 E_{11}=  \varphi*E_{f,11}+\left( 1-\varphi \right) * E_{M}
\end{equation}
...
\noindent Tabelle ~\ref{bh} enthält Werte der Gurtquerschnittsfläche bei verschiedenen Kombinationen von $ b $ und $ h $, die zum erforderlichen gesamten Flächenträgheitsmoment von $ I_{x,min} = 3,04722 * 10^{-8} m^{4} $ führen.\\
\begin{table}[h]
...
\begin{equation}
	\label{gurtlagen}
	n=t_{soll}*\frac{\varphi*\rho_{f}}{\left(\frac{m_{f}}{L*b}\right)}
\end{equation}
