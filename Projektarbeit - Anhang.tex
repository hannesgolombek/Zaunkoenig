...
%3
\begin{equation}
	S_{z,3}(s_3) = t\int_{0}^{s_3}\bigg(37,5\mathrm{mm}+14\mathrm{mm}\bigg)ds = t\bigg[51,5\mathrm{mm}\cdot s\bigg]_{s=0}^{s_3} = t\cdot\bigg(51,5\mathrm{mm}\cdot s_2\bigg)
\end{equation}
\begin{equation}
	S_{y,3}(s_3) = t\int_{0}^{s_3}s\ ds= t\bigg[\frac{1}{2}s^2\bigg]_{s=0}^{s_3} = t\cdot\bigg(\frac{1}{2}s^2_3\bigg)
\end{equation}
%4
\begin{equation}
\begin{split}
...
\end{equation}
	\item Hebelarme um den Pol unter Berücksichtigung des Positiven Drehsinnes
		...
		\end{tabular}
	\end{center}
	\item Integrale über Statische Momente
	\begin{equation}
...
~\\
	\begin{equation}
		\begin{split}
			\int S_{\bar z,1}\ ds_1 &= \int  tR^2\cdot\bigg(\frac{s_1}{R}-sin\bigg(\frac{s_1}{R}-\frac{y_0s_1}{R^2}\bigg)-\frac{y_0s_1}{R^2}\bigg)\ ds_1
		\end{split}
	\end{equation}
\end{itemize}
\newpage
